...
class ShieldAccessibilityService : AccessibilityService() {
    ...
    private fun triggerBlock(matchedText: String) {
        ...
        // Prefer the real system-overlay window: it renders above
        // Samsung's pop-up/mini-window (multi-window) mode, split-screen,
        // and PIP, none of which a plain Activity can guarantee it's on
        // top of. Only fall back to the Activity if the user hasn't
        // granted "draw over other apps" yet, so blocking still works
        // (for normal full-screen apps) before that permission is set up.
    ...
    override fun onInterrupt() {
        // System interrupted the service (e.g. another accessibility
        // service is taking over) — drop any overlay we're holding so we
        // don't leak a window the service can no longer manage.
        if (::blockOverlayWindow.isInitialized) {
